...
def perf() -> None:
    ...
    village = Village.create_village(population_count=10)
    print(f"pop: 10\t\t\t{timeit.timeit(village.tick, number=1)}")

    village = Village.create_village(population_count=1_000)
    print(f"pop: 1.000\t\t{timeit.timeit(village.tick, number=1)}")

    village = Village.create_village(population_count=100_000)
